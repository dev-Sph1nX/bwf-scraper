...
// ===== Journal des exécutions (data/run-log.json) =====
// Une ligne par run, y compris les runs "à vide" (aucun nouveau match) : c'est
// ce qui permet d'afficher un vrai suivi quotidien des mises à jour, même quand
// rien de neuf n'a été récupéré. Fichier versionné (comme le reste de data/).
const RUN_LOG_PATH = join(DATA_DIR, "run-log.json");
const RUN_LOG_MAX = 400;

export async function appendRunLog(record) {
  let log = [];
  if (existsSync(RUN_LOG_PATH)) {
    try { log = await readJson(RUN_LOG_PATH); } catch { log = []; }
  }
  if (!Array.isArray(log)) log = [];
  log.push(record);
  if (log.length > RUN_LOG_MAX) log = log.slice(-RUN_LOG_MAX);
  await writeJson(RUN_LOG_PATH, log);
}

export async function getRunLog() {
  if (!existsSync(RUN_LOG_PATH)) return [];
  try {
    const log = await readJson(RUN_LOG_PATH);
