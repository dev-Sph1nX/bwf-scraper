 ...
 */
export function matchOdds(bwfCandidates, oddsRows, { includeSettled = false } = {}) {
  const matched = [], ambiguous = [], unmatchedOdds = [], noOdds = [], settled = [];
  const usedBwf = new Set();

  for (const row of oddsRows) {
    // Un match déjà décidé n'est pas « à venir » : par défaut on l'écarte, pour
    // que le rapport d'audit ne parle que de ce sur quoi on peut encore parier.
    //
    // Mais `includeSettled` permet de les apparier : leur cote est la DERNIÈRE
    // observée, donc la meilleure approximation de la cote de clôture — et la
    // cote de clôture est la référence du marché, celle qui intègre toute
    // l'information disponible. C'est le seul étalon qui dise si notre modèle
    // bat les bookmakers, et donc s'il peut gagner de l'argent. Écarter ces
    // lignes revenait à jeter la mesure la plus précieuse du projet.
    if (row.settled && !includeSettled) { settled.push(row); continue; }
