...
// --- Marché « total de points » ------------------------------------------------
//
// Betclic est le SEUL opérateur français à coter les totaux de points BWF
// (marché lancé en déc. 2024 — run 4 du bureau d'études, bwf-playground).
// Le libellé exact du marché sur la page match n'a encore jamais été observé
// dans un relevé (le run 4, partie C, a montré que ce dépôt ne l'a jamais
// capté) : le parseur accepte donc les formes plausibles — un escalier de
// barres dans un marché unique (« Nombre total de points », labels
// « Plus/Moins de 75,5 ») ou un marché par barre (« Total de points 75,5 »,
// labels nus « Plus »/« Moins », barre en fin de nom). Les totaux PAR SET
// sont écartés. À VÉRIFIER au premier relevé d'un tournoi coté, et resserrer.

/**
 * Lignes over/under du total de points du MATCH, même schéma que Pinnacle :
 * [{n, over, under}] trié par barre, cote ≤ 1 neutralisée (null).
 * @param {Array<{name: string, selections: Array<{label: string, odd: number|null}>}>} markets
 * @returns {Array<{n: number, over: number|null, under: number|null}>|null} null si non coté
 */
export function totalsFromBetclicMarkets(markets) {
  const parBarre = new Map();
  for (const mk of markets || []) {
    const nom = String(mk.name || "");
    if (!/total.*points?|points?.*total/i.test(nom)) continue;
    if (/\b(sets?|manches?|jeux?)\b/i.test(nom)) continue; // « du 1er set », etc.
    const barreDuNom = nom.match(/(\d+(?:[.,]\d+)?)\s*$/); // barre en fin de nom
    for (const s of mk.selections || []) {
      const lab = String(s.label || "").trim();
      const m = lab.match(/^(plus|moins)(?:\s+de\s+(\d+(?:[.,]\d+)?))?\s*$/i);
      if (!m) continue;
      const n = Number(String(m[2] ?? barreDuNom?.[1] ?? "").replace(",", "."));
      if (!Number.isFinite(n)) continue;
      const e = parBarre.get(n) || { n, over: null, under: null };
      e[/^plus/i.test(m[1]) ? "over" : "under"] = s.odd ?? null;
      parBarre.set(n, e);
    }
  }
  const totals = [...parBarre.values()]
    .filter((l) => l.over != null || l.under != null)
    .sort((a, b) => a.n - b.n);
  return totals.length ? totals : null;
}

/**
 * Complète les lignes prématch avec les marchés de la page match : « nombre
 * de sets » (champ `sets`) et « total de points » (champ `totals`) — voir
 * lib/books.mjs. Une seule lecture de page par match pour les deux marchés.
 * BEST-EFFORT : toute page match illisible est sautée — le relevé du
 * vainqueur n'en dépend jamais.
 * @param {object[]} rows lignes prématch de fetchBetclic (mutées en place)
 * @returns {Promise<number>} nombre de lignes enrichies (sets ou totals)
 */
export async function enrichBetclicSets(rows, _ctx, { pauseMs = 600, maxMatches = 40, baseUrl = PAGE_URL } = {}) {
  ...
  for (const row of (rows || []).slice(0, maxMatches)) {
    ...
    try {
      ...
      if (sets) row.sets = sets;
      const totals = totalsFromBetclicMarkets(markets);
      if (totals) row.totals = totals;
      if (sets || totals) n++;
