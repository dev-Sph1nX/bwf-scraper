...
import { recalibrate } from "./calibrate.mjs";
...
export const MODELS = [
  ...
  },
  {
    ...
    label: "Elo recalibré",
    binary: false,
    // Même modèle que ci-dessus, dont les probabilités sont étirées là où un
    // défaut de calibration est DÉMONTRÉ (cf. lib/calibrate.mjs). Il est ajouté
    // comme modèle à part plutôt qu'en modifiant l'Elo : c'est le duel qui doit
    // dire s'il améliore, pas nous. Le taux de réussite est identique par
    // construction — seule la confiance annoncée change.
    predict: (r) => {
      if (isProvisional(r.nA) || isProvisional(r.nB)) return null;
      if (!Number.isFinite(r.eloA) || !Number.isFinite(r.eloB)) return null;
      return recalibrate(eloProb(r.eloA, r.eloB), r.disc);
    },
  },
];
