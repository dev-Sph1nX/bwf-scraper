...
export class BwfClient {
   ...
   *
   * On parse d'abord le CORPS HTTP BRUT de la navigation : l'API sert son JSON
   * en content-type text/html, et certaines réponses (constaté sur les tournois
   * 2022, ex. team_meeting contenant "<p><strong>…") embarquent du HTML brut
   * dans leurs chaînes — Chromium les parse alors comme du HTML et l'innerText
   * rend un JSON invalide. L'innerText reste en repli (corps indisponible).
   * @param {string} url
   * @returns {Promise<any>}
   */
  async getJson(url) {
    ...
    for (let attempt = 0; attempt < 4; attempt++) {
      try {
        ...
        /* corps illisible ou non-JSON (défi Cloudflare) : on tente l'innerText */
      }
      ...
      try {
        ...
        response = await this.#page.reload({ waitUntil: "domcontentloaded", timeout: 60000 });
      }
    }
