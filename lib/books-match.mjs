...
export function matchBooks(bwfCandidates, groups) {
  ...
  for (const g of groups) {
    ...
    // Proximité de date exigée SEULEMENT quand la BWF a publié l'horaire :
    // 91 % des matchs à venir n'en ont pas encore (l'ordre de jeu paraît la
    // veille) — les exclure rendrait presque toutes les cotes orphelines.
    // Sans horaire, la décision repose sur le score de joueurs et la marge
    // anti-ambiguïté, comme partout ailleurs.
    const pool = bwfCandidates.filter(
      (c) => c.eventName === g.discipline && (!c.matchTime || dayGap(c.matchTime, g.startUtc) <= 1),
    );
    ...
    // Clé d'unicité du match BWF construite sur les IDS DES JOUEURS, présents
    // dans les deux populations de candidats. Surtout pas c.a/c.b : les
    // candidats joués n'en ont pas, et « undefined|undefined » faisait entrer
    // en collision tous les matchs joués d'un même tournoi (vu en production).
    const ids = (t) => (t?.players || []).map((p) => String(p.id)).sort().join("-");
    const cle = `${best.c.tournamentName}|${best.c.eventName}|${best.c.matchTime}|${ids(best.c.team1)}|${ids(best.c.team2)}`;
