...
export async function getSummary(year) {
  ...
  if (yearData) {
    ...
    for (const t of tournaments) {
      ...
      for (const d of (Array.isArray(draws.results) ? draws.results : [])) {
...
export async function getStatus(year) {
  ...
  for (const t of flatten(yearData)) {
    ...
    tournaments.push({
      ...
      drawsTotal: Array.isArray(draws?.results) ? draws.results.length : 0, drawCount, matchCount,
    });
  }
...
export async function getTournament(year, tmtId) {
  ...
  if (draws) {
    for (const d of (Array.isArray(draws.results) ? draws.results : [])) {
      ...
      const grid = data ? data.results ?? {} : {};
      // Poule (round-robin) : la grille n'a pas de 1re colonne "0-…" (pas un arbre).
      // Dans ce cas on expose la liste plate des matchs pour l'afficher autrement.
      const gridKeys = Object.keys(grid);
      const isGroup = gridKeys.length > 0 && !gridKeys.some((k) => /^0-/.test(k));
      disciplines.push({
        ...
        group: isGroup,
        matches: isGroup ? (data.matches ?? []) : undefined,
      });
    }
  }
