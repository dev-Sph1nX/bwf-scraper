...
/** IC 95 % bootstrap sur la MOYENNE d'une liste de valeurs (graine fixe). */
function bootstrapMeanCI(values, { draws = 500, seed = 42 } = {}) {
  ...
  means.sort((a, b) => a - b);
  return [round4(means[Math.floor(0.025 * (draws - 1))]), round4(means[Math.ceil(0.975 * (draws - 1))])];
}
...
export function aggregate(bets, { draws = 500, seed = 42 } = {}) {
  ...
  const ci = bootstrapMeanCI(bets.map((b) => b.gain), { draws, seed });
  return { n, staked: n, net: round2(net), roi: round4(net / n), ci, won };
}
...
export function computeRoi(rows, { draws = 500, seed = 42 } = {}) {
  ...
  // (helpers réutilisés par le découpage par discipline : ils régénèrent les
  // paris depuis un sous-ensemble de lignes, à l'identique du journal)
  const bandsOf = (subset) => {
    const out = BANDS.map((band) => ({ band }));
    for (const instant of INSTANTS) {
      const groups = new Map(BANDS.map((b) => [b, []]));
      for (const r of subset) for (const b of favoriBets(r, instant)) {
        groups.get(bandOf(b.side === 1 ? b.rowProb : 100 - b.rowProb)).push(b);
      }
      for (const e of out) e[instant] = aggregate(groups.get(e.band), opts);
    }
    return out;
  };
  const sweepOf = (subset) => {
    const vBets = {};
    for (const instant of INSTANTS) vBets[instant] = subset.flatMap((r) => valueBets(r, instant));
    return EV_THRESHOLDS.map((threshold) => {
      const e = { threshold };
      for (const instant of INSTANTS) {
        e[instant] = aggregate(vBets[instant].filter((b) => b.ev > threshold), opts);
      }
      return e;
    });
  };
  const bands = bandsOf(usable);

  // --- analyse 4 : balayage du seuil d'EV (refiltre les paris value, ev connu) ---
  const evSweep = sweepOf(usable);
  ...
  // --- avenant : CLV — les paris d'ouverture battent-ils la clôture ? ---
  // clv = cote prise à l'ouverture / meilleure cote de clôture du même camp − 1.
  // clv > 0 : le marché a fini par nous donner raison (signal d'avantage réel,
  // visible bien avant que le ROI ne sorte du bruit).
  const clv = {};
  for (const key of Object.keys(gen)) {
    const items = [];
    for (const r of usable) {
      for (const b of gen[key](r, "open")) {
        const c = bestOddAt(r.books, b.side, "close");
        if (c) items.push({ clv: b.odd / c.odd - 1, gain: b.gain, won: b.won });
      }
    }
    const beat = items.filter((x) => x.clv > 0);
    clv[key] = {
      n: items.length,
      beat: beat.length,
      beatPct: items.length ? round4(beat.length / items.length) : null,
      avg: items.length ? round4(items.reduce((s, x) => s + x.clv, 0) / items.length) : null,
      ci: bootstrapMeanCI(items.map((x) => x.clv), opts),
      roiBeat: aggregate(beat, opts),
      roiOther: aggregate(items.filter((x) => x.clv <= 0), opts),
    };
  }

  // --- avenant : découpage par discipline + croisements tranches/seuil d'EV ---
  const DISC_ORDER = ["MS", "WS", "MD", "WD", "XD"];
  const presents = [...new Set(usable.map((r) => r.disc))];
  const discs = DISC_ORDER.filter((d) => presents.includes(d))
    .concat(presents.filter((d) => !DISC_ORDER.includes(d)));
  const byDisc = discs.map((disc) => {
    const dRows = usable.filter((r) => r.disc === disc);
    const entry = { disc, bands: bandsOf(dRows), evSweep: sweepOf(dRows) };
    for (const key of Object.keys(gen)) {
      entry[key] = {};
      for (const instant of INSTANTS) {
        entry[key][instant] = aggregate(dRows.flatMap((r) => gen[key](r, instant)), opts);
      }
    }
    return entry;
  });

  return { totalMatches: usable.length, strategies, bands, evSweep, disagreement, byBook, clv, byDisc, bets: betLog };
}
