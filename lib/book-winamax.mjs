...
// match. Trois formes relevées :
//   - marketId 196 « Nombre exact de sets », issues « 2 » / « 3 » (tennis) ;
//   - marketId 241 « Nombre de sets », special `variant=sr:exact_games:bestof:3`,
//     issues « 2 » / « 3 » — c'est la forme du BADMINTON (Championnats du
//     monde, 2026-08-18) : titre SANS « exact », id différent du tennis ;
//   - marketId 314 « Nombre de sets », issues « Plus/Moins de 2,5 »
//     (équivalent strict en best of 3).
//
// Les ids de marché changent donc d'un sport à l'autre : on ne s'y fie pas
// seul. Un bet est retenu s'il parle de SETS au niveau du MATCH, et c'est
// `setsFromOutcomes` (qui n'accepte que « 2 »/« 3 » et « Plus/Moins de 2,5 »)
// qui valide réellement le contenu.

const MARKET_EXACT_SETS = 196;
const MARKET_OVER_UNDER_SETS = 314;
const MARKET_SETS_BADMINTON = 241;
const SETS_MARKET_IDS = new Set([MARKET_EXACT_SETS, MARKET_OVER_UNDER_SETS, MARKET_SETS_BADMINTON]);

/**
 * Ce bet est-il un marché « nombre de sets » DU MATCH ?
 * Rejette les marchés d'un set précis (`gamenr=`, ex. « 1er set - Nombre de
 * points ») et les over/under sur un autre total que 2,5 (le best of 5 du
 * tennis cote « Moins de 3,5 sets », qui n'est pas « match en 2 sets »).
 */
function isMatchSetsBet(bet) {
  const special = String(bet?.specialBetValue || "");
  if (/gamenr=/i.test(special)) return false;
  if (/\btotal=/i.test(special) && !/\btotal=2[.,]5\b/i.test(special)) return false;
  return SETS_MARKET_IDS.has(bet?.marketId)
    || /^nombre (exact )?de (sets|manches)\b/i.test(bet?.betTitle || "");
}

/** Le marché « exact » (issues « 2 »/« 3 ») passe avant l'over/under 2,5. */
function isExactSetsBet(bet) {
  return bet?.marketId === MARKET_EXACT_SETS
    || /exact_games/i.test(String(bet?.specialBetValue || ""))
    || /^nombre exact de (sets|manches)\b/i.test(bet?.betTitle || "");
}
...
export function parseWinamaxMatchSets(state, matchId) {
  ...
  // On essaie TOUS les marchés de sets du match (et pas seulement le premier
  // trouvé) : un marché suspendu peut n'avoir aucune cote payante.
  const candidates = bets.filter(isMatchSetsBet)
    .sort((a, b) => Number(isExactSetsBet(b)) - Number(isExactSetsBet(a)));
 ...
 */
export async function enrichWinamaxSets(rows, _ctx, { pauseMs = 600, maxMatches = 57 } = {}) {
  let n = 0;
