 ...
 * @param {object} [opts]
 * @param {boolean} [opts.requireOdds=true]  par défaut, seuls les matchs
 *   Flashscore portant au moins une cote de clôture sont joints (c'est le
 *   contrat historique, celui d'export/cotes.csv). À false, les matchs sans
 *   cote joignent aussi (books vide) : le pont fsId ↔ match sert alors à
 *   d'autres données du même match (point par point), pas qu'aux cotes.
 * @returns {{ joined: Map<string, object>, stats: object }}
 ...
 *   fsId, swap }. `swap` = true quand le « home » Flashscore est notre team2 :
 *   indispensable pour lire les marchés orientés du même match (score exact)
 *   stockés ailleurs en orientation Flashscore.
 */
export function joinFlashscore(fsFiles, bwfRows, { requireOdds = true } = {}) {
  ...
  for (const file of fsFiles) {
    for (const m of file.matches) {
      if (!m.sets?.length || (requireOdds && !m.odds)) continue;
      stats.fsMatches++;
      ...
      if (!Object.keys(books).length && requireOdds) { stats.unmatched++; continue; }
      const cle = `${r.tmtId}|${r.disc}|${r.day}|${r.a}|${r.b}`;
      // Deux entrées Flashscore peuvent viser le même match BWF (doublon d'un
      // tournoi re-slugué) : celle qui porte des cotes prime, jamais l'inverse.
      if (joined.has(cle) && !Object.keys(books).length) continue;
      joined.set(cle, {
        books,
        ...
        // par cette structure.
        fsId: m.fsId ?? null,
        // ORIENTATION RETENUE, et pourquoi elle est publiée : les cotes
        // ci-dessus sont déjà retournées si besoin, mais les AUTRES marchés du
        // même match (score exact 2-0 / 0-2 dans data/flashscore/sets/) sont
        // stockés dans l'orientation Flashscore. Sans ce drapeau, un
        // consommateur qui lit ces marchés croit son « 2-0 » aligné sur notre
        // team1 alors qu'il décrit le home Flashscore — soit notre team2 quand
        // swap vaut true. Le lire est obligatoire pour les marchés orientés.
        swap,
      });
      stats.joined++;
    }
